...
export async function fetchCandles(symbol,timeframe,outputsize=500,signal){
 const key=cacheKey(symbol,timeframe,outputsize),cached=candleCache.get(key);
 ...
 const response=await fetch(`${API_BASE}/time_series?${params}`,{signal}),body=await response.json();
...
export function mergeLiveBarIntoCache(symbol,timeframe,bar,outputsize=500){
 const key=cacheKey(symbol,timeframe,outputsize),cached=candleCache.get(key);if(!cached)return;
 const bars=cached.bars.slice(),last=bars[bars.length-1];if(last?.time===bar.time)bars[bars.length-1]={...bar};else if(!last||bar.time>last.time)bars.push({...bar});
 while(bars.length>outputsize)bars.shift();candleCache.set(key,{savedAt:Date.now(),bars});
}

class TwelveDataSocketManager{
 constructor(){this.socket=null;this.listeners=new Map();this.reconnectTimer=null;this.idleTimer=null;this.heartbeat=null;this.retry=0;this.intentionalClose=false;}
 subscribe(symbol,onTick,onError){
  const tdSymbol=toTwelveSymbol(symbol);this.cancelIdleClose();let set=this.listeners.get(tdSymbol);const firstForSymbol=!set;if(!set){set=new Set();this.listeners.set(tdSymbol,set);}const listener={onTick,onError};set.add(listener);
  this.ensureConnected();if(firstForSymbol&&this.socket?.readyState===WebSocket.OPEN)this.sendSubscription('subscribe',tdSymbol);
  return()=>{const current=this.listeners.get(tdSymbol);if(!current)return;current.delete(listener);if(!current.size){this.listeners.delete(tdSymbol);if(this.socket?.readyState===WebSocket.OPEN)this.sendSubscription('unsubscribe',tdSymbol);}if(!this.listeners.size)this.scheduleIdleClose();};
 }
 ensureConnected(){if(!this.listeners.size||this.socket?.readyState===WebSocket.OPEN||this.socket?.readyState===WebSocket.CONNECTING)return;this.intentionalClose=false;this.socket=new WebSocket(`${WS_BASE}?apikey=${encodeURIComponent(apiKey())}`);
  ...
  this.socket.addEventListener('message',event=>{try{const data=JSON.parse(event.data);if(data?.event!=='price')return;const price=Number(data.price),time=Number(data.timestamp);if(!Number.isFinite(price)||!Number.isFinite(time))return;const targets=this.listeners.get(data.symbol);if(!targets)return;const tick={price,time:Math.floor(time),symbol:data.symbol};for(const listener of targets)listener.onTick(tick);}catch(error){this.emitError(error);}});
 ...
 disconnect(){this.intentionalClose=true;this.cancelIdleClose();if(this.reconnectTimer){window.clearTimeout(this.reconnectTimer);this.reconnectTimer=null;}this.stopHeartbeat();const socket=this.socket;this.socket=null;if(socket&&socket.readyState<2)socket.close();}
}
