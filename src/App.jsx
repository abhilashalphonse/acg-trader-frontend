  ...
  return (
    <div className="terminal-shell">
      <header className="topbar">
        ...
        <button className="mobile-menu" type="button" onClick={() => setMobileWatch((value) => !value)}><SlidersHorizontal size={17} /></button>
      </header>
      ...
      <div className="toolbar">
        ...
        <button className="timeframe-more" type="button"><ChevronDown size={13} /></button>
        <div className="toolbar-actions"><button type="button" onClick={() => setDepthOpen((value) => !value)} className={depthOpen ? 'toolbar-active' : ''}>Depth</button><button type="button" onClick={() => setOrderOpen(true)}>New Order</button><button type="button"><Crosshair size={14} /></button><button type="button"><Settings2 size={14} /></button></div>
      </div>

      <main className="workspace">
        <aside className={`watchlist-panel ${mobileWatch ? 'mobile-open' : ''}`}>
          <div className="panel-heading"><div><span className="eyebrow">MARKET WATCH</span><strong>Forex CFDs</strong></div><button type="button" className="small-button" onClick={() => setMobileWatch(false)}><X size={15} /></button></div>
        ...
        <section className="center-column">
          <div className="chart-panel">
            <div className="chart-header"><div><div className="instrument-title"><strong>{market.symbol}</strong><span>Forex CFD</span><span className="live-pill">LIVE</span></div><div className="price-line"><span className="bid-price">{formatPrice(market.bid, market.digits)}</span><span className="slash">/</span><span className="ask-price">{formatPrice(market.ask, market.digits)}</span><span className="change">{market.change}%</span></div></div><div className="chart-tools"><button type="button"><Crosshair size={15} /></button><button type="button"><SlidersHorizontal size={15} /></button><button type="button"><BarChart3 size={15} /></button><button type="button"><Maximize2 size={15} /></button></div></div>
            ...
            <div className="quick-trade-bar"><div className="quick-label">ONE-CLICK TRADING</div><button className="quick-sell" type="button" onClick={() => setOrderSide('Sell')}><small>SELL</small><strong>{formatPrice(market.bid, market.digits)}</strong></button><div className="quick-volume"><button type="button" onClick={() => changeVolume(-0.01)}>−</button><strong>{volume.toFixed(2)}</strong><span>lots</span><button type="button" onClick={() => changeVolume(0.01)}>+</button></div><button className="quick-buy" type="button" onClick={() => setOrderSide('Buy')}><small>BUY</small><strong>{formatPrice(market.ask, market.digits)}</strong></button></div>
          </div>
