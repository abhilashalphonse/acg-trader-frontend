...
export default function App() {
  ...
  const goMobile = (view) => { setMobileView(view); setMarketOpen(false); setSettingsOpen(false) }
  ...
  const selectSymbol = (symbol) => { setSelectedSymbol(symbol); setMobileView('Chart'); setMarketOpen(false) }

  return <div className={`acg-app ${darkMode ? 'acg-dark' : ''}`}>
...
function MobileTerminal({ view, market, selectedSymbol, setSelectedSymbol, timeframe, setTimeframe, volume, changeVolume, pulse, quickOrder, filteredMarkets, favorites, toggleFavorite, watchFilter, setWatchFilter, search, setSearch, setOrderSide, setOrderOpen, terminalTab, setTerminalTab, darkMode, toggleTheme, goMobile }) {
  return <div className="mobile-terminal">
    ...
    <nav className="mobile-bottom-nav" aria-label="Primary"><MobileNavButton active={view === 'Quotes'} icon={<BarChart3 size={19} />} label="Quotes" onClick={() => goMobile('Quotes')} /><MobileNavButton active={view === 'Chart'} icon={<CandlestickChart size={19} />} label="Chart" onClick={() => goMobile('Chart')} /><MobileNavButton active={view === 'Trade'} icon={<SlidersHorizontal size={19} />} label="Trade" onClick={() => goMobile('Trade')} /><MobileNavButton active={view === 'History'} icon={<History size={19} />} label="History" onClick={() => goMobile('History')} /><MobileNavButton active={view === 'Settings'} icon={<Settings2 size={19} />} label="Settings" onClick={() => goMobile('Settings')} /></nav>
  </div>
}

function MobileChart({ market, selectedSymbol, timeframe, setTimeframe, volume, changeVolume, pulse, quickOrder }) {
  const [menuOpen, setMenuOpen] = useState(false)
  return <section className="mobile-chart-screen"><div className="mobile-chart-toolbar"><button type="button" aria-label="Menu" onClick={() => setMenuOpen((value) => !value)}><Menu size={19} /></button><button type="button" className="mobile-toolbar-active" aria-label="Chart type"><CandlestickChart size={18} /></button><button type="button" aria-label="Indicators"><BarChart3 size={18} /></button><button type="button" className="mobile-timeframe">{timeframe}<span>⌄</span></button><button type="button" aria-label="Objects"><Link2 size={18} /></button><button type="button" aria-label="Drawing tools"><Activity size={19} /></button></div><div className="mobile-quote-strip"><button className="mobile-quote-sell" type="button" onClick={() => quickOrder('Sell')}><span>SELL</span><strong style={tabular} className={pulse ? 'quote-pulse' : ''}>{price(market.bid, market.digits)}</strong></button><div className="mobile-lot"><button type="button" onClick={() => changeVolume(-0.01)}><ChevronRight size={15} style={{ transform: 'rotate(180deg)' }} /></button><strong style={tabular}>{volume.toFixed(2)}</strong><button type="button" onClick={() => changeVolume(0.01)}><ChevronRight size={15} /></button></div><button className="mobile-quote-buy" type="button" onClick={() => quickOrder('Buy')}><span>BUY</span><strong style={tabular} className={pulse ? 'quote-pulse' : ''}>{price(market.ask, market.digits)}</strong></button></div><div className="mobile-chart-title">{market.symbol}, {timeframe}: {market.name}</div><div className="mobile-chart-canvas"><TradingChart symbol={selectedSymbol} timeframe={timeframe} chartType="candles" showPriceAxis gridLines /><div className="mobile-price-tag" style={tabular}>{price(market.bid, market.digits)}</div></div><div className="mobile-chart-timeframes">{timeframes.slice(0, 6).map((tf) => <button key={tf} type="button" className={timeframe === tf ? 'active' : ''} onClick={() => setTimeframe(tf)}>{tf}</button>)}</div>{menuOpen && <div className="mobile-chart-menu"><strong>Chart</strong><button type="button">Indicators</button><button type="button">Objects</button><button type="button">Full screen</button></div>}</section>
}
