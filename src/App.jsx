...
// Single source of truth for the positive/negative price color so the chart,
// the watchlist, and the mobile views can never drift out of sync with each other.
...
export default function App() {
  ...
  const visibleMarkets = watchFilter === 'favorites' ? searchedMarkets.filter((m) => favorites.has(m.symbol)) : searchedMarkets
  ...
  return <div className={`acg-app ${darkMode ? 'acg-dark' : 'acg-light'}`}>
    <style>{`
      ...
      .watch-filter-tabs { display:flex;gap:14px;padding:0 12px 8px;border-bottom:1px solid #e2e7ee; }
      .acg-dark .watch-filter-tabs { border-color:var(--acg-border); }
      .watch-filter-tab { border:0;background:transparent;font-size:11px;font-weight:600;color:#8a96a5;padding-bottom:7px;cursor:pointer; }
    ...
    <div className="main-grid">
      ...
      <aside className={`market-watch ${marketOpen ? 'mobile-open' : ''}`}><div className="watch-head"><div><span>MARKET WATCH</span><strong>Forex CFDs</strong></div><button type="button" className="close-watch mobile-only" onClick={() => setMarketOpen(false)}><X size={18} /></button></div><label className="symbol-search"><Search size={15} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search symbol" /></label><div className="watch-filter-tabs"><button type="button" className={`watch-filter-tab ${watchFilter === 'favorites' ? 'active' : ''}`} onClick={() => setWatchFilter('favorites')}>Favorites</button><button type="button" className={`watch-filter-tab ${watchFilter === 'all' ? 'active' : ''}`} onClick={() => setWatchFilter('all')}>All</button></div><div className="watch-cols"><span /><span>Symbol</span><span>Bid</span><span>Ask</span><span>Daily Ch...</span></div><div className="watch-list">{visibleMarkets.map((m) => {
        ...
        return <div key={m.symbol} role="button" tabIndex={0} className={`watch-row ${selectedSymbol === m.symbol ? 'selected' : ''}`} onClick={selectSymbol} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && selectSymbol()}>
          ...
          <span style={tabularNums}>{price(m.bid, m.digits)}</span>
          <span style={tabularNums}>{price(m.ask, m.digits)}</span>
          <span style={tabularNums} className={changeClass(m.change)}>{m.change}</span>
        </div>
      })}{watchFilter === 'favorites' && visibleMarkets.length === 0 && <div className="empty-state"><strong>No favorites yet</strong><span>Tap the star on a symbol to pin it here.</span></div>}</div></aside>
    </div>
...
function MobileView({ view, visibleMarkets, favorites, toggleFavorite, watchFilter, setWatchFilter, selectedSymbol, search, setSearch, setSelectedSymbol, setMobileView, terminalTab, setTerminalTab, setOrderSide, setOrderOpen, setSettingsOpen, darkMode, toggleTheme }) {
  ...
  const filterTab = (key, label) => <button type="button" onClick={() => setWatchFilter(key)} style={{ border: 0, background: 'transparent', fontSize: 11, fontWeight: 600, color: watchFilter === key ? '#1769e0' : '#8a96a5', borderBottom: watchFilter === key ? '2px solid #1769e0' : '2px solid transparent', padding: '0 0 7px' }}>{label}</button>
  if (view === 'Quotes') return <section className="mobile-view-surface" style={shell}><div style={header}><strong style={title}>Quotes</strong>{close}</div><div style={{ padding: 10 }}><label className="symbol-search" style={{ margin: 0 }}><Search size={15} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search symbol" /></label></div><div style={{ display: 'flex', gap: 14, padding: '0 10px 8px', borderBottom: `1px solid ${darkMode ? '#2a3544' : '#e2e7ee'}` }}>{filterTab('favorites', 'Favorites')}{filterTab('all', 'All')}</div><div style={{ padding: '0 10px' }}>{visibleMarkets.map((m) => { const isFav = favorites.has(m.symbol); return <div key={m.symbol} role="button" tabIndex={0} onClick={() => { setSelectedSymbol(m.symbol); setMobileView('Chart') }} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && setSelectedSymbol(m.symbol)} style={{ width: '100%', display: 'grid', gridTemplateColumns: 'auto 1fr auto', alignItems: 'center', gap: 8, textAlign: 'left', border: 0, borderBottom: `1px solid ${darkMode ? '#2a3544' : '#eef1f5'}`, background: selectedSymbol === m.symbol ? (darkMode ? '#193453' : '#edf5ff') : (darkMode ? '#151c26' : '#fff'), color: darkMode ? '#e6ebf2' : '#273246', padding: '10px 8px', cursor: 'pointer' }}><button type="button" className={`fav-star ${isFav ? 'active' : ''}`} onClick={(e) => toggleFavorite(m.symbol, e)} aria-label={isFav ? `Remove ${m.symbol} from favorites` : `Add ${m.symbol} to favorites`}><Star size={13} fill={isFav ? 'currentColor' : 'none'} /></button><span><strong style={{ display: 'block', fontSize: 11 }}>{m.symbol}</strong><small style={{ display: 'block', marginTop: 3, fontSize: 8, color: darkMode ? '#8f9bab' : '#96a1b0' }}>{m.name}</small></span><span style={{ ...tabularNums, textAlign: 'right', fontFamily: 'ui-monospace, monospace', fontSize: 9 }}><strong style={{ display: 'block' }}>{price(m.bid, m.digits)}</strong><span>{price(m.ask, m.digits)} · </span><span className={changeClass(m.change)}>{m.change}</span></span></div> })}{watchFilter === 'favorites' && visibleMarkets.length === 0 && <div style={{ padding: '24px 8px', textAlign: 'center', fontSize: 11, color: '#8a96a5' }}>Tap the star on a symbol to pin it here.</div>}</div></section>
