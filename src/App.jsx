    ...
    <div className="main-grid">
      ...
      <section className="terminal-workspace">
        <div className="chart-toolbar">
          <div className="view-modes"><button className={chartType === 'candles' ? 'active' : ''} type="button" onClick={() => setChartType('candles')} aria-label="Candles"><CandlestickChart size={15} /></button><button className={chartType === 'line' ? 'active' : ''} type="button" onClick={() => setChartType('line')} aria-label="Line"><LineChart size={15} /></button></div>
...
function MobileView({ view, filteredMarkets, selectedSymbol, search, setSearch, setSelectedSymbol, setMobileView, terminalTab, setTerminalTab, setOrderSide, setOrderOpen, setSettingsOpen }) {
  if (view === 'Chart') return null
  const shell = { position: 'fixed', inset: '48px 0 62px', zIndex: 20, background: '#fff', overflow: 'auto' }
  const header = { height: 48, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 12px', borderBottom: '1px solid #e2e7ee', position: 'sticky', top: 0, zIndex: 2 }
  const title = { fontSize: 14, fontWeight: 800, color: '#273246' }
  const close = <button type="button" onClick={() => setMobileView('Chart')} style={iconButton}><X size={17} /></button>
  if (view === 'Quotes') return <section style={shell}><div style={header}><strong style={title}>Quotes</strong>{close}</div><div style={{ padding: 10 }}><label className="symbol-search" style={{ margin: 0 }}><Search size={15} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search symbol" /></label></div><div style={{ display: 'flex', gap: 6, padding: '0 10px 10px', overflowX: 'auto' }}>{['Favorites', 'Forex', 'Metals', 'Indices'].map((x, i) => <button key={x} type="button" style={pill(i === 0)}>{x}</button>)}</div><div style={{ padding: '0 10px' }}>{filteredMarkets.map((m) => <button key={m.symbol} type="button" onClick={() => { setSelectedSymbol(m.symbol); setMobileView('Chart') }} style={{ width: '100%', display: 'grid', gridTemplateColumns: '1fr auto', textAlign: 'left', border: 0, borderBottom: '1px solid #eef1f5', background: selectedSymbol === m.symbol ? '#edf5ff' : '#fff', padding: '12px 8px' }}><span><strong style={{ display: 'block', fontSize: 11, color: '#273246' }}>{m.symbol}</strong><small style={{ display: 'block', marginTop: 3, fontSize: 8, color: '#96a1b0' }}>{m.name}</small></span><span style={{ textAlign: 'right', fontFamily: 'ui-monospace, monospace', fontSize: 9 }}><strong style={{ display: 'block', color: '#273246' }}>{price(m.bid, m.digits)}</strong><span>{price(m.ask, m.digits)} · </span><span className={m.change.startsWith('-') ? 'red-text' : 'blue-text'}>{m.change}</span></span></button>)}</div></section>
  if (view === 'Trade') return <section style={{ ...shell, background: '#f5f7fa' }}><div style={header}><strong style={title}>Trade</strong><button type="button" onClick={() => { setOrderSide('Buy'); setOrderOpen(true) }} style={primary}>+ New Order</button></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8, padding: 10 }}><Metric label="Balance" value="$100,000.00" /><Metric label="Equity" value="$100,000.00" /><Metric label="Free Margin" value="$100,000.00" /><Metric label="Margin Level" value="—" /></div><div style={card}><div style={{ display: 'flex', borderBottom: '1px solid #e5e9ef' }}>{['Positions', 'Orders'].map((tab) => <button key={tab} type="button" onClick={() => setTerminalTab(tab)} style={{ ...tabButton, color: terminalTab === tab ? '#1769e0' : '#788598', fontWeight: terminalTab === tab ? 800 : 500 }}>{tab}</button>)}</div><EmptyState label={terminalTab === 'Positions' ? 'You don’t have any positions' : 'You don’t have any pending orders'} /></div><div style={{ ...card, padding: 12 }}><div style={{ fontSize: 9, color: '#8a96a5', fontWeight: 800, letterSpacing: '.7px' }}>ACG CHALLENGE</div><div style={{ marginTop: 6, fontSize: 15, fontWeight: 800, color: '#273246' }}>● SAFE</div><div style={{ marginTop: 10, display: 'grid', gap: 7 }}><RiskRow label="Daily Loss" value="$42 / $500" /><RiskRow label="Max Loss" value="$51 / $1,000" /><RiskRow label="Profit Target" value="$0 / $1,000" /></div></div></section>
  if (view === 'History') return <section style={shell}><div style={header}><strong style={title}>History</strong><span style={{ fontSize: 9, color: '#8a96a5' }}>Today</span></div><div style={{ padding: '0 10px' }}>{history.map((r) => <button key={r.ticket} type="button" style={{ width: '100%', textAlign: 'left', border: 0, borderBottom: '1px solid #eef1f5', background: '#fff', padding: '13px 8px' }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><strong style={{ fontSize: 11, color: '#273246' }}>{r.symbol}</strong><strong className="profit" style={{ fontSize: 10 }}>{r.profit}</strong></div><div style={{ marginTop: 5, display: 'flex', gap: 10, fontSize: 8, color: '#7b8797' }}><span>{r.type} {r.volume}</span><span>{r.price}</span><span>{r.time}</span></div></button>)}</div></section>
  return <section style={{ ...shell, background: '#f5f7fa' }}><div style={header}><strong style={title}>More</strong>{close}</div><div style={{ padding: 10, display: 'grid', gap: 8 }}>{['Challenge & Risk', 'Alerts', 'Economic Calendar', 'Trade Journal', 'Account', 'Settings', 'Help'].map((item, i) => <button key={item} type="button" onClick={() => i === 0 ? setMobileView('Trade') : i === 5 ? setSettingsOpen(true) : undefined} style={{ ...card, margin: 0, minHeight: 54, padding: '0 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', textAlign: 'left', color: '#273246', fontWeight: 700, fontSize: 10 }}>{item}<span style={{ color: '#9aa5b3' }}>›</span></button>)}</div></section>
}
...
const card = { margin: '10px', border: '1px solid #e2e7ee', borderRadius: 8, overflow: 'hidden', background: '#fff' }
const tabButton = { flex: 1, height: 38, border: 0, background: '#fff', fontSize: 9 }
const pill = (active) => ({ border: 0, borderRadius: 14, padding: '6px 10px', whiteSpace: 'nowrap', background: active ? '#1769e0' : '#f3f6f9', color: active ? '#fff' : '#6f7d8f', fontSize: 8, fontWeight: 800 })
function Metric({ label, value }) { return <div style={{ border: '1px solid #e2e7ee', borderRadius: 7, background: '#fff', padding: 10 }}><div style={{ fontSize: 7, color: '#8a96a5', textTransform: 'uppercase', letterSpacing: '.5px' }}>{label}</div><strong style={{ display: 'block', marginTop: 5, fontSize: 10, color: '#273246' }}>{value}</strong></div> }
function RiskRow({ label, value }) { return <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 8 }}><span style={{ color: '#7b8797' }}>{label}</span><strong style={{ color: '#354156' }}>{value}</strong></div> }
