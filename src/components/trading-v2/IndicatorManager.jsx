...
export default function IndicatorManager({
  ...
  return (
    <div>
      <div className="flex h-11 items-center gap-2 rounded-xl border border-[#1b2d3d] bg-[#0a151f] px-3">
        ...
        <section className="mt-3">
          ...
          <div className="space-y-1.5">
            {applied.map(indicator => {
              ...
              return (
                <div key={indicator.instanceId} className="overflow-hidden rounded-xl border border-[#1b3040] bg-[#09151f]">
                  <div className="flex items-center gap-2 px-3 py-2.5">
                    <button type="button" onClick={() => onToggleVisible(indicator.instanceId)} className={`grid size-6 shrink-0 place-items-center rounded-md border text-[10px] ${indicator.visible !== false ? 'border-[#245579] bg-[#123149] text-[#65cfff]' : 'border-[#263746] bg-[#0c1822] text-[#526779]'}`} aria-label="Toggle indicator visibility">{indicator.visible !== false ? <Check size={12}/> : '—'}</button>
      ...
      <div className="mt-4 flex gap-1.5 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {categories.map(item => <button key={item} type="button" onClick={() => setCategory(item)} className={`h-8 shrink-0 rounded-lg px-3 text-[9px] font-bold ${category === item ? 'border border-[#285474] bg-[#10283a] text-[#61caff]' : 'border border-[#182b3a] bg-[#08131c] text-[#71869a]'}`}>{item}</button>)}
      </div>
      ...
      <section className="mt-2 space-y-1.5">
        {available.map(definition => {
          ...
          return (
            <div key={definition.id} className="flex items-center gap-2 rounded-xl border border-[#182b3a] bg-[#08131c] px-3 py-2.5">
              ...
              <button type="button" onClick={() => onAdd(definition.id)} className="h-8 rounded-lg border border-[#245070] bg-[#0d2638] px-3 text-[9px] font-black text-[#62caff]">ADD</button>
            </div>
          );
        })}
...
function NumberField({ label, value, min = 1, max = 500, step = 1, onChange }) {
  return <label className="block"><span className="mb-1 block text-[7px] font-bold uppercase tracking-[0.08em] text-[#64798d]">{label}</span><input type="number" min={min} max={max} step={step} value={value} onChange={event => onChange(Number(event.target.value))} className="h-9 w-full rounded-lg border border-[#203240] bg-[#071019] px-2.5 text-[10px] font-semibold text-[#dbe5ed] outline-none focus:border-[#2b5d7e]" /></label>;
}
...
function SettingsPanel({ indicator, onUpdate }) {
  ...
  if (!fields) return <div className="border-t border-[#172837] px-3 py-2.5 text-[8px] text-[#667b8e]">This indicator has no adjustable parameters.</div>;
  return <div className="border-t border-[#172837] bg-[#071019]/70 px-3 py-2.5">{fields}</div>;
}
