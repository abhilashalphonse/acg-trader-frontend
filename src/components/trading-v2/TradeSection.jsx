  ...
  return (
    <section className="min-h-[calc(100dvh-98px)] px-3 pb-6 pt-3">
      <header className="flex items-start justify-between gap-3 pb-4">
        ...
        <button type="button" onClick={onNewOrder} className="mt-1 flex h-10 items-center gap-1.5 rounded-xl border border-white/[0.08] bg-[#101010] px-3 text-[9px] font-extrabold text-[#62cbff]"><Plus size={14}/>New order</button>
      </header>

      <div className="rounded-[22px] border border-white/[0.08] bg-[#080808] p-4 shadow-[0_18px_50px_rgba(0,0,0,.24)]">
      ...
      <div className="mt-2 space-y-2">
        ...
        {positions.map(position => {
          ...
          return (
            <article key={position.id} className="overflow-hidden rounded-[18px] border border-white/[0.08] bg-[#080808]">
              ...
              {expanded && <div className="border-t border-white/[0.08] bg-[#080808] px-3.5 pb-3.5 pt-3">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => onOpenChart(position.symbol)} className="flex h-10 items-center justify-center gap-2 rounded-xl border border-white/[0.08] bg-[#101010] text-[9px] font-bold text-[#63cbff]"><ExternalLink size={13}/>View on chart</button><button type="button" onClick={() => onClosePosition(position.id, 100)} className="flex h-10 items-center justify-center gap-2 rounded-xl border border-[#562c35] bg-[#251319] text-[9px] font-bold text-[#ff7a85]"><X size={13}/>Close position</button></div>
              </div>}
            </article>
          );
        })}
      ...
      <div className="mt-2 space-y-2">
        ...
        {pendingOrders.map(order => {
          ...
          return <article key={order.id} className="rounded-[17px] border border-white/[0.08] bg-[#080808] px-3.5 py-3"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><InstrumentAvatar instrument={marketFor(order.symbol)} size={24}/><strong className="text-[12px] font-black text-[#eff4f8]">{symbolLabel(order.symbol)}</strong><span className={`rounded-md px-1.5 py-1 text-[7px] font-black ${sideTone(side)}`}>{side} {String(order.orderType || 'order').toUpperCase()}</span></div><p className="mt-2 text-[9px] text-[#71859a]">{Number(order.lots || order.manualLots || 0).toFixed(2)} lots · Entry <b className="font-mono text-[#c1ccd6]">{price(order.entry, order.symbol)}</b></p><div className="mt-2 flex gap-3 text-[8px] text-[#60758a]"><span>SL <b className="text-[#9eb0bf]">{price(order.sl, order.symbol)}</b></span><span>TP <b className="text-[#9eb0bf]">{price(order.tp, order.symbol)}</b></span></div></div></div><div className="mt-3 grid grid-cols-3 gap-2"><button type="button" onClick={() => onOpenChart(order.symbol)} className="h-9 rounded-xl border border-white/[0.08] bg-[#101010] text-[8px] font-bold text-[#5fc9ff]">Chart</button><button type="button" onClick={() => onModifyPending(order.id)} className="h-9 rounded-xl border border-white/[0.08] bg-[#080808] text-[8px] font-bold text-[#b5c3ce]">Modify</button><button type="button" onClick={() => onCancelPending(order.id)} className="h-9 rounded-xl border border-[#512b34] bg-[#101010] text-[8px] font-bold text-[#ff7984]">Cancel</button></div></article>;
        })}
      </div>
    </section>
  );
...
function EmptyState({ title, subtitle, compact = false }) {
  return <div className={`grid place-items-center rounded-[18px] border border-dashed border-white/[0.08] bg-[#080808] text-center ${compact ? 'h-[92px]' : 'h-[118px]'}`}><div><b className="text-[10px] text-[#9eafbe]">{title}</b><p className="mt-1 text-[8px] text-[#5d7185]">{subtitle}</p></div></div>;
}
