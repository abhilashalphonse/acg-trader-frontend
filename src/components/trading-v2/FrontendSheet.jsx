...
export default function FrontendSheet({ type, onClose, markets = [], activeSymbol, onSelectSymbol = () => {} }) {
    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/55 px-2 backdrop-blur-[2px]" onMouseDown={onClose}>
      <section onMouseDown={event => event.stopPropagation()} className="mb-[max(8px,env(safe-area-inset-bottom))] max-h-[78dvh] w-full max-w-[444px] overflow-hidden rounded-[24px] border border-[#203343] bg-[#071019] shadow-[0_30px_90px_rgba(0,0,0,.65)]">
        ...
        <div className="max-h-[calc(78dvh-70px)] overflow-y-auto p-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {type === 'indicators' && <div className="space-y-1.5">{indicators.map(name => { const active = selectedIndicators.includes(name); return <button key={name} type="button" onClick={() => setSelectedIndicators(current => active ? current.filter(item => item !== name) : [...current, name])} className={`flex w-full items-center justify-between rounded-xl border px-3 py-3 text-left ${active ? 'border-[#245070] bg-[#0c2536]' : 'border-[#182b3a] bg-[#08131c]'}`}><span><b className="block text-[11px] text-[#eaf1f6]">{name}</b><small className="mt-1 block text-[8px] text-[#6c8094]">{active ? 'Selected in frontend' : 'Tap to select'}</small></span><span className={`grid size-5 place-items-center rounded-md text-[10px] ${active ? 'bg-[#1a4f6e] text-[#6ed0ff]' : 'bg-[#101d27] text-[#53697b]'}`}>{active ? '✓' : '+'}</span></button>})}</div>}
