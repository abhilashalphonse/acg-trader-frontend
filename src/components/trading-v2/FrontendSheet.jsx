...
export default function FrontendSheet({
  ...
  const renderMarkets = favoritesOnly => {
    ...
    if (!source.length) return <div className="grid min-h-36 place-items-center rounded-xl border border-white/[0.08] bg-[#080808] px-5 text-center text-[9px] text-[#687d91]">No instruments match this search.</div>;
    return <div className="space-y-1.5">{list.map(item => {
      const isWatched = watched.has(item.symbol);
      return <div key={item.symbol} className={`flex items-center gap-2 rounded-xl border px-2 py-2 ${activeSymbol === item.symbol ? 'border-white/[0.13] bg-[#101010]' : 'border-white/[0.08] bg-[#080808]'}`}><button type="button" onClick={() => { onSelectSymbol(item.symbol); onClose(); }} className="flex min-w-0 flex-1 items-center justify-between px-1 py-1 text-left"><div className="flex min-w-0 items-center gap-2.5"><InstrumentAvatar instrument={item} size={32}/><div className="min-w-0"><div className="flex items-center gap-2"><strong className="text-[12px] text-[#eef4f8]">{item.displaySymbol || symbolLabel(item.symbol)}</strong><span className="rounded-md bg-[#101010] px-1.5 py-0.5 text-[7px] font-bold text-[#738ba0]">{marketGroup(item)}</span></div><p className="mt-1 truncate text-[9px] text-[#6f8296]">{item.name || 'Market instrument'}</p></div></div><div className="ml-3 text-right"><b className="block text-[11px] text-[#dce5ec]">{item.subscribed ? (item.bid ?? '—') : '—'}</b><span className={`mt-1 block text-[8px] ${item.live ? 'text-[#31d79d]' : item.isStale ? 'text-[#e8c35f]' : 'text-[#71869a]'}`}>{item.subscribed ? (item.sessionOpen === false ? 'Closed' : item.live ? 'Live' : item.isStale ? 'Stale' : item.marketState || 'Waiting') : 'Open for quote'}</span></div></button><button type="button" onClick={() => watchlists?.toggleSymbol?.(item.symbol)} aria-label={isWatched ? 'Remove from watchlist' : 'Add to watchlist'} className={`grid size-9 shrink-0 place-items-center rounded-lg ${isWatched ? 'text-[#f6c95d]' : 'text-[#60758a] hover:bg-white/[0.04] hover:text-[#f6c95d]'}`}><Star size={16} fill={isWatched ? 'currentColor' : 'none'}/></button></div>;
    })}{source.length > list.length && <button type="button" onClick={() => setVisibleMarketLimit(limit => limit + 60)} className="h-10 w-full rounded-xl border border-white/[0.08] bg-[#101010] text-[9px] font-bold text-[#69cfff]">Show more · {source.length - list.length} remaining</button>}</div>;
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/55 px-2 backdrop-blur-[2px]" onMouseDown={onClose}>
      <section onMouseDown={event => event.stopPropagation()} className={`${isIndicators ? 'max-h-[90dvh]' : 'max-h-[84dvh]'} mb-[max(8px,env(safe-area-inset-bottom))] w-full max-w-[444px] overflow-hidden rounded-[24px] border border-white/[0.08] bg-[#080808] shadow-[0_30px_90px_rgba(0,0,0,.65)]`}>
        <header className="flex items-center justify-between border-b border-white/[0.08] px-4 py-3.5"><div><h2 className="text-[15px] font-black text-[#f3f7fb]">{titleMap[type] || 'ACG Trader'}</h2><p className="mt-0.5 text-[9px] text-[#6e8195]">{isIndicators ? 'Technical studies · live chart' : type === 'more' ? 'Profiles, preferences and shortcuts' : type === 'notifications' ? 'Terminal and risk alerts' : type === 'profile' ? 'Trading account summary' : type === 'help' ? 'Using ACG Trader' : 'Markets and terminal tools'}</p></div><button type="button" onClick={onClose} className="grid size-9 place-items-center rounded-xl border border-white/[0.08] bg-[#101010] text-[#93a6b8]"><X size={17}/></button></header>
        <div className={`${isIndicators ? 'max-h-[calc(90dvh-70px)]' : 'max-h-[calc(84dvh-70px)]'} overflow-y-auto p-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden`}>
          {message && <div className="mb-3 rounded-xl border border-white/[0.08] bg-[#101010] px-3 py-2.5 text-[9px] leading-4 text-[#a9c5d8]">{message}</div>}
          {(type === 'search' || type === 'instruments' || type === 'markets') && <><div className="mb-2 flex h-11 items-center gap-2 rounded-xl border border-white/[0.08] bg-[#080808] px-3"><Search size={16} className="text-[#6f8295]"/><input autoFocus={type === 'search'} value={query} onChange={e => setQuery(e.target.value)} placeholder="Search 300 markets…" className="min-w-0 flex-1 bg-transparent text-[12px] text-[#eef4f8] outline-none placeholder:text-[#53677b]"/></div><div className="mb-3 flex gap-1.5 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{MARKET_GROUPS.map(group => <button key={group} type="button" onClick={() => setMarketGroupFilter(group)} className={`shrink-0 rounded-lg border px-2.5 py-1.5 text-[8px] font-bold ${marketGroupFilter === group ? 'border-white/[0.13] bg-[#101010] text-[#62cbff]' : 'border-white/[0.08] bg-[#080808] text-[#71869a]'}`}>{group}</button>)}</div>{renderMarkets(false)}</>}
          {type === 'watchlist' && renderMarkets(true)}
          {type === 'notifications' && <div className="space-y-2"><div className="rounded-xl border border-white/[0.08] bg-[#080808] p-3"><div className="flex items-start gap-2"><Bell size={16} className="mt-0.5 text-[#55c7ff]"/><div><b className="text-[11px] text-[#edf3f7]">Terminal alerts</b><p className="mt-1 text-[9px] leading-4 text-[#73869a]">Execution, connection and market-state alerts appear automatically while you trade.</p></div></div></div><div className="rounded-xl border border-white/[0.08] bg-[#080808] p-3"><div className="flex items-start gap-2"><ShieldCheck size={16} className="mt-0.5 text-[#42d7a2]"/><div><b className="text-[11px] text-[#edf3f7]">Risk alerts</b><p className="mt-1 text-[9px] leading-4 text-[#73869a]">Challenge limits and exposure blocks are shown beside the order ticket.</p></div></div></div></div>}
          {type === 'profile' && <div className="space-y-2"><div className="flex items-center gap-3 rounded-2xl border border-white/[0.08] bg-[#080808] p-3"><div className="grid size-11 place-items-center rounded-full bg-[#101010] text-[#67ccff]"><UserRound size={21}/></div><div className="min-w-0"><b className="block truncate text-[12px] text-[#f0f5f9]">{account.accountCode || 'Trading account'}</b><p className="mt-1 text-[9px] text-[#718398]">{account.accountType || 'ACG Trader'} · {account.status || 'UNKNOWN'}</p></div></div><div className="grid grid-cols-2 gap-2"><Pref label="Currency" value={account.currency || 'USD'}/><Pref label="Leverage" value={account.leverage ? `1:${account.leverage}` : '—'}/><Pref label="Valuation" value={account.valuationStatus || 'Waiting'}/><Pref label="Trading" value={account.tradingEnabled === false ? 'Disabled' : 'Enabled'}/></div></div>}
          ...
          {type === 'more' && (
            <div className="space-y-4">
              <section>
                ...
                <div className="grid grid-cols-2 gap-2">{tradingProfiles.map(profile => <button key={profile.id} type="button" onClick={() => { onApplyTradingProfile(profile); setMessage(`${profile.name} profile applied.`); }} className="rounded-xl border border-white/[0.08] bg-[#080808] p-3 text-left active:bg-[#101010]"><b className="text-[10px] text-[#edf3f7]">{profile.name}</b><p className="mt-1.5 text-[8px] leading-4 text-[#6f8396]">{profile.description}</p></button>)}</div>
              </section>

              <section className="rounded-2xl border border-white/[0.08] bg-[#080808] p-3">
