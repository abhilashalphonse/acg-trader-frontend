        ...
        <>
          ...
          <div className="absolute right-2 top-[54px] z-[90] w-[min(318px,calc(100vw-16px))] overflow-hidden rounded-xl border border-white/[0.10] bg-[#0b0b0d] shadow-[0_22px_70px_rgba(0,0,0,.70)]">
            ...
            <div className="max-h-[55dvh] overflow-y-auto">
              {accounts.map(item => {
                ...
                return (
                  <button
                    ...
                    onClick={() => chooseAccount(id)}
                    disabled={accountSwitching && !selected}
                    className={`flex w-full items-center gap-2.5 border-b border-white/[0.06] px-3 py-3 text-left last:border-b-0 disabled:cursor-wait disabled:opacity-45 ${selected ? 'bg-white/[0.045]' : 'bg-transparent active:bg-white/[0.035]'}`}
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        <strong className="truncate text-[10px] font-black text-[#eef4f8]">{item.accountCode || 'Trading account'}</strong>
              ...
              {!accounts.length && <div className="px-3 py-4 text-center text-[8px] text-[#687783]">No trading accounts are available.</div>}
            </div>
          </div>
        </>
      )}
