...
export default function AccountSection({ account = {}, onOpenSheet = () => {} }) {
  ...
  return (
    <section className="min-h-[calc(100dvh-98px)] px-3 pb-6 pt-3">
      ...
      <div className="rounded-[22px] border border-white/[0.08] bg-[#080808] p-4 shadow-[0_18px_50px_rgba(0,0,0,.22)]">
        ...
        <div className="mt-2 flex items-center justify-between rounded-xl border border-white/[0.08] bg-[#101010] px-3 py-2.5"><span className="text-[8px] font-bold uppercase tracking-[0.09em] text-[#5d7287]">Valuation</span><span className={`text-[9px] font-black ${valuation === 'LIVE' ? 'text-[#45dda9]' : valuation === 'STALE' ? 'text-[#e8c35f]' : 'text-[#90a2b4]'}`}>{valuation}</span></div>
      </div>
      ...
      <div className="mt-2 overflow-hidden rounded-[18px] border border-white/[0.08] bg-[#080808]">
...
function Stat({ label, value }) { return <div className="rounded-xl border border-white/[0.08] bg-[#101010] px-3 py-2.5"><span className="block text-[7px] font-bold uppercase tracking-[0.1em] text-[#5d7287]">{label}</span><b className="mt-1 block text-[11px] text-[#d6e0e8]">{value}</b></div>; }
