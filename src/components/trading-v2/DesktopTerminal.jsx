...
export default function DesktopTerminal({
  ...
  const manualOrder = side => {
    if (onManualOrder) {
      onManualOrder({ side, lots, price: side === 'buy' ? market?.ask : market?.bid, symbol: market?.symbol });
      return;
    }
    notify(`Frontend demo: ${side.toUpperCase()} ${lots.toFixed(2)} ${market?.symbol}`);
  };
  ...
  return (
    <div ref={shellRef} className="relative h-dvh min-h-[720px] overflow-hidden bg-[#02070c] text-[#f4f8fb]">
      ...
      <header className="flex h-16 items-center border-b border-[#172737] bg-[#060d14] px-4 shadow-[0_1px_0_rgba(255,255,255,0.015)]">
        ...
        <div className="ml-3 hidden items-stretch divide-x divide-[#172737] rounded-xl border border-[#172737] bg-[#08111a] xl:flex">{[['Balance', '$12,458.32'], ['Equity', '$12,503.18'], ['P/L', '+$44.86'], ['Free Margin', '$11,982.40']].map(([label, value]) => <div key={label} className="min-w-[112px] px-3 py-2"><span className="block text-[8px] font-semibold uppercase tracking-[0.08em] text-[#52667a]">{label}</span><strong className={`mt-0.5 block text-[11px] font-bold ${label === 'P/L' ? 'text-[#3dd9a4]' : 'text-[#dce6ef]'}`}>{value}</strong></div>)}</div>
            ...
            {tab === 'positions' && <>
              ...
              {positions.length ? positions.map(position => {
                ...
                return <div key={position.id} className="relative grid grid-cols-[1.08fr_.48fr_.52fr_.78fr_.68fr_.68fr_.62fr_210px] items-center border-b border-[#111f2c] px-3 py-2 text-[9px] text-[#aab9c8]">
                  ...
                  <span className="font-mono">{formatPrice(position.entry)}</span>
                  <span className="font-mono">{formatPrice(position.sl)}</span>
                  <span className="font-mono">{formatPrice(position.tp)}</span>
                  <b className={Number(position.pnl) >= 0 ? 'text-[#3dd9a4]' : 'text-[#ff6570]'}>{formatPnl(position.pnl)}</b>
                  <div className="flex items-center justify-end gap-1">
                    <button type="button" onClick={() => onClosePosition(position.id, 50)} className="h-7 rounded-md border border-[#22333f] bg-[#0b1720] px-2 text-[7px] font-bold text-[#aebbc7]">50%</button>
                    <button type="button" onClick={() => onBreakEven(position.id)} className="h-7 rounded-md border border-[#1d4639] bg-[#0c2019] px-2 text-[7px] font-bold text-[#48d7a4]">BE</button>
                    <button type="button" onClick={() => onReversePosition(position.id)} className="h-7 rounded-md border border-[#244156] bg-[#0b1c28] px-2 text-[7px] font-bold text-[#5fc9ff]">REV</button>
                    <button type="button" onClick={() => onClosePosition(position.id, 100)} className="h-7 rounded-md border border-[#5b2931] bg-[#241217] px-2 text-[7px] font-bold text-[#ff7882]">CLOSE</button>
                    <button type="button" onClick={() => setPositionMenuId(menuOpen ? null : position.id)} className="grid size-7 place-items-center rounded-md border border-[#22333f] bg-[#0b1720] text-[#708398]"><MoreHorizontal size={13}/></button>
                  </div>
                  {menuOpen && <div className="absolute right-3 top-9 z-30 w-[218px] rounded-xl border border-[#213543] bg-[#09131d] p-2 shadow-xl">
                    <div className="grid grid-cols-2 gap-1.5">
                      <button type="button" onClick={() => onDuplicatePosition(position.id)} className="flex h-8 items-center justify-center gap-1.5 rounded-lg border border-[#1c3241] bg-[#0b1822] text-[8px] font-bold text-[#b9c6d1]"><Copy size={11}/>Duplicate</button>
                      <button type="button" onClick={() => onSetTrailing(position.id, !position.trailingEnabled, position.trailingPips || 5)} className={`flex h-8 items-center justify-center gap-1.5 rounded-lg border text-[8px] font-bold ${position.trailingEnabled ? 'border-[#24506a] bg-[#0d293b] text-[#62ccff]' : 'border-[#1c3241] bg-[#0b1822] text-[#b9c6d1]'}`}><ShieldCheck size={11}/>Trail {position.trailingEnabled ? 'On' : 'Off'}</button>
                    </div>
                    <div className="mt-1.5 grid grid-cols-3 gap-1.5"><button type="button" onClick={() => onClosePosition(position.id, 25)} className="h-7 rounded-lg bg-[#171117] text-[7px] font-bold text-[#cbbbc1]">Close 25%</button><button type="button" onClick={() => onClosePosition(position.id, 75)} className="h-7 rounded-lg bg-[#171117] text-[7px] font-bold text-[#cbbbc1]">Close 75%</button><button type="button" onClick={() => onReversePosition(position.id)} className="flex h-7 items-center justify-center gap-1 rounded-lg bg-[#0b1a24] text-[7px] font-bold text-[#67cfff]"><Repeat2 size={10}/>Reverse</button></div>
                  </div>}
                </div>;
              }) : <div className="grid h-[100px] place-items-center text-[9px] text-[#607489]">No open positions</div>}
            </>}

            {tab === 'orders' && <div className="grid h-[130px] place-items-center text-[9px] text-[#607489]">Pending orders are managed from the mobile order planner for now</div>}
            {tab === 'history' && <div className="overflow-y-auto">{positionHistory.length ? positionHistory.map((row, index) => <div key={`${row.id}-${index}`} className="flex items-center justify-between border-b border-[#111f2c] px-4 py-3 text-[9px]"><span><b className="text-[#dce7f1]">{displaySymbol(row.symbol)}</b><span className="ml-2 text-[#64788d]">{row.side} {Number(row.volume).toFixed(2)} · {row.closeType || 'Closed'}</span></span><span className={Number(row.pnl) >= 0 ? 'text-[#3dd9a4]' : 'text-[#ff6570]'}>{formatPnl(row.pnl)} · {row.closedAt}</span></div>) : <div className="grid h-[130px] place-items-center text-[9px] text-[#607489]">Closed frontend positions appear here</div>}</div>}
          </div>
        </section>
      </div>
    </div>
