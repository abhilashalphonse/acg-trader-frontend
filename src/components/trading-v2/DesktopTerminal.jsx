    ...
    <div ref={shellRef} className="relative h-dvh min-h-0 overflow-hidden bg-[#02070c] text-[#f4f8fb]">
      ...
      <div className="grid h-[calc(100dvh-64px)] min-h-[656px] grid-cols-[58px_230px_minmax(0,1fr)] 2xl:grid-cols-[62px_270px_minmax(0,1fr)]">
        ...
        <section className="grid min-h-0 grid-rows-[60px_36px_44px_minmax(0,1fr)_104px_250px] bg-[#060d14] 2xl:grid-rows-[62px_36px_46px_minmax(0,1fr)_108px_260px]">
          <div className="flex items-center border-b border-[#172737] bg-[#08111a] px-4"><div className="flex min-w-[240px] items-center gap-2.5"><InstrumentAvatar instrument={market} size={32}/><div className="min-w-0"><button type="button" onClick={() => searchRef.current?.focus()} className="flex items-center gap-1 text-[15px] font-extrabold tracking-[-0.025em] text-[#f3f7fb]">{market?.displaySymbol || displaySymbol(market?.symbol)}<ChevronDown size={14}/></button><span className="mt-1 block truncate text-[8px] text-[#5f7388]">{marketLabel(market)}</span></div></div><div className="ml-4"><strong className="block font-mono text-[17px] tracking-[-0.02em] text-[#edf5fb]">{market?.bid || '—'}</strong><span className={`mt-1 block text-[8px] font-semibold ${market?.live ? 'text-[#35d49f]' : market?.isStale ? 'text-[#e7bd58]' : 'text-[#718398]'}`}>{market?.sessionOpen === false ? 'SESSION CLOSED' : market?.live ? 'LIVE' : market?.isStale ? 'STALE' : market?.marketState || 'WAITING'}</span></div><div className="ml-auto flex items-center gap-4"><div className="hidden text-right xl:block"><span className="block text-[7px] uppercase tracking-[0.08em] text-[#506477]">Valuation</span><b className={`mt-1 block text-[9px] ${valuationStatus === 'LIVE' ? 'text-[#3dd9a4]' : valuationStatus === 'STALE' ? 'text-[#e7bd58]' : 'text-[#a0b0bf]'}`}>{valuationStatus}</b></div><button type="button" onClick={() => watchlists?.toggleSymbol?.(activeSymbol)} className={`grid size-8 place-items-center rounded-lg hover:bg-white/[0.035] ${favorite ? 'text-[#f6c95d]' : 'text-[#687d92]'}`}><Star size={16} fill={favorite ? 'currentColor' : 'none'}/></button></div></div>

          <PropRiskStrip account={account} plannedRisk={plannedRisk} compact />

          <div className="flex items-center gap-2 border-b border-[#172737] bg-[#071019] px-3"><div className="flex items-center gap-0.5 rounded-lg border border-[#1a2c3c] bg-[#09131d] p-1">{timeframes.map(([label, value]) => <button key={value} type="button" onClick={() => onTimeframeChange(value)} disabled={Boolean(tradePlan && !tradePlan.open)} className={`h-7 min-w-8 rounded-md px-2 text-[8px] font-bold ${timeframe === value ? 'bg-[#123249] text-[#58c7ff]' : 'text-[#6d8298] hover:bg-white/[0.035] hover:text-[#d7e2ec]'} disabled:opacity-30`}>{label}</button>)}</div><div className="h-5 w-px bg-[#1b2c3b]"/><div className="flex items-center gap-0.5 rounded-lg border border-[#1a2c3c] bg-[#09131d] p-1"><button type="button" onClick={() => onChartModeChange('candles')} disabled={Boolean(tradePlan && !tradePlan.open)} className={`grid size-7 place-items-center rounded-md ${chartMode === 'candles' ? 'bg-[#123249] text-[#58c7ff]' : 'text-[#6d8298]'} disabled:opacity-30`} title="Candlesticks"><CandlestickChart size={15}/></button><button type="button" onClick={() => onChartModeChange('line')} disabled={Boolean(tradePlan && !tradePlan.open)} className={`grid size-7 place-items-center rounded-md ${chartMode === 'line' ? 'bg-[#123249] text-[#58c7ff]' : 'text-[#6d8298]'} disabled:opacity-30`} title="Line chart"><ChartNoAxesCombined size={15}/></button><button type="button" onClick={onOpenIndicators} className={`relative grid size-7 place-items-center rounded-md text-[10px] font-black hover:text-white ${indicators.length ? 'bg-[#102b3d] text-[#5bc9ff]' : 'text-[#6d8298]'}`} title="Indicators">ƒx{indicators.length > 0 && <span className="absolute -right-1 -top-1 grid size-3.5 place-items-center rounded-full bg-[#1e5a7d] text-[6px] text-white">{indicators.length}</span>}</button></div><button type="button" onClick={toggleFullscreen} className="ml-auto grid size-8 place-items-center rounded-lg border border-[#1a2c3c] bg-[#09131d] text-[#73889d] hover:text-white" title="Fullscreen"><Maximize2 size={15}/></button></div>

          <div className="min-h-0 min-w-0 bg-[#080f17]"><ChartArea symbol={market?.symbol} instrument={market} chartTimeframe={chartTimeframeMap[timeframe] || 'M1'} tick={tick} price={market?.bid} ask={market?.ask} chartMode={chartMode} selectedTool={selectedTool} onSelectTool={onSelectedToolChange} embedded tradePlan={tradePlan} onTradePlanChange={onTradePlanChange} onUpdatePosition={onUpdatePosition} indicators={indicators} positions={positions}/></div>

          <div className="border-t border-[#172737] bg-[#071019] px-2 pb-2"><ExecutionPanel market={market} account={account} exposureAllowed={exposureAllowed} exposureBlockReason={exposureBlockReason} focusMode lots={lots} onLotsChange={onLotsChange} sizingMode={sizingMode} onSizingModeChange={onSizingModeChange} riskPercent={riskPercent} onRiskPercentChange={onRiskPercentChange} orderType={orderType} onOrderTypeChange={onOrderTypeChange} tradePlan={tradePlan} onStartPlan={onStartPlan} onCancelPlan={onCancelPlan} onExecutePlan={onExecutePlan} onModifyPlan={onModifyPlan} onManualOrder={submitOneClick} onTradePlanChange={onTradePlanChange}/></div>

          <div className="min-h-0 overflow-y-auto border-t border-[#172737] bg-[#050c13] px-2 pb-2"><PositionsPanel positions={positions} markets={markets} positionHistory={positionHistory} pendingOrders={pendingOrders} journal={journal} onClosePosition={onClosePosition} onCloseAll={onCloseAllPositions} onBreakEven={onBreakEven} onReverse={onReversePosition} onUpdatePosition={onUpdatePosition} onSetTrailing={onSetTrailing} onDuplicate={onDuplicatePosition} onCancelPending={onCancelPending} onModifyPending={onModifyPending}/></div>
        </section>
      </div>
    </div>
  );
