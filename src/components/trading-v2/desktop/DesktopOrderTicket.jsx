...
export default function DesktopOrderTicket({
  ...
  return (
    <section className="min-h-0 border-t border-white/[0.06] bg-[#07090B]">
      <div className="flex h-9 items-center justify-between border-b border-white/[0.06] px-2.5">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="text-[8px] font-bold text-[#A1AFBC]">{market?.displaySymbol || market?.symbol || '—'}</span>
          <span className={`text-[6px] font-black ${market?.live ? 'text-[#38d6a2]' : 'text-[#697c8e]'}`}>● {liveLabel}</span>
        </div>
        <span className="text-[6.5px] text-[#6F8191]">{orderType === 'market' && sizingMode === 'lots' ? '1-click' : 'planned'}</span>
      </div>
      ...
      <div className="space-y-1.5 overflow-y-auto px-2 py-2 [scrollbar-width:thin]">
        <div className="grid grid-cols-4 gap-1">
          {ORDER_TYPES.map(([id, label]) => (
            <button
              ...
              className={`h-8 rounded border text-[9px] font-semibold transition ${orderType === id ? 'border-[#315b72] bg-[#0d1a22] text-[#59C7FF]' : 'border-white/[0.06] bg-black text-[#6F8191] hover:text-[#d2dce5]'}`}
            >
              {label}
            </button>
          ))}
        ...
        <div className="flex items-center gap-2">
          <span className="shrink-0 text-[6px] font-black uppercase tracking-[0.08em] text-[#6F8191]">Size by</span>
          <div className="flex flex-1 items-center rounded-md border border-white/[0.06] bg-black p-0.5">
            <button type="button" onClick={() => setMode('lots')} className={`h-7 flex-1 rounded text-[7px] font-bold ${sizingMode === 'lots' ? 'bg-white/[0.055] text-[#E6EDF3]' : 'text-[#627589]'}`}>Lots</button>
            <button type="button" onClick={() => setMode('risk')} className={`h-6 flex-1 rounded text-[7px] font-bold ${sizingMode === 'risk' ? 'bg-white/[0.055] text-[#E6EDF3]' : 'text-[#627589]'}`}>Risk %</button>
          </div>
        </div>

        {sizingMode === 'lots' ? (
          <>
            <div className="grid grid-cols-[30px_minmax(0,1fr)_30px] items-center rounded-md border border-white/[0.06] bg-black">
              ...
              <div className="flex items-center justify-center border-x border-white/[0.06]">
                <input
                  ...
                  className="h-9 w-20 bg-transparent text-center font-mono text-[14px] font-black tabular-nums text-[#E6EDF3] outline-none"
                  inputMode="decimal"
                  aria-label="Lot size"
                />
        ...
        {sizingMode === 'risk' && (
          <div className="flex h-6 items-center justify-between rounded border border-white/[0.05] bg-black px-2 text-[6.5px] text-[#6F8191]">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          ...
          <button type="button" disabled={!canSubmit} onClick={() => clickSide('buy')} className={`flex h-[50px] min-w-0 flex-col items-end justify-center rounded-md border px-2.5 text-right transition active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-35 ${selectedSide === 'buy' ? 'border-[#21845f] bg-[#071710]' : 'border-[#1c5c47] bg-black hover:bg-[#06110d]'}`}>
            <span className="text-[7px] font-black tracking-[0.08em] text-[#42D7A1]">BUY</span>
        ...
        <div className="-mt-0.5 text-center text-[6px] font-semibold text-[#6F8191]">Spread {Number.isFinite(spreadPips) ? `${spreadPips.toFixed(1)}p` : '—'}</div>

        <div className="rounded-md border border-white/[0.06] bg-[#0C1013] p-1.5">
          <div className="mb-1.5 flex items-center justify-between">
            <strong className="text-[8px] font-semibold uppercase tracking-[0.08em] text-[#A1AFBC]">Protection</strong>
            {!pendingPlan && (
              <div className="flex items-center gap-1">
                <button type="button" disabled={!executableQuote || !exposureAllowed} onClick={() => startProtectedPlan('sell')} className="h-7 rounded border border-[#5b252e] px-1.5 text-[6px] font-bold text-[#FF6F7A] disabled:opacity-30">SELL plan</button>
                <button type="button" disabled={!executableQuote || !exposureAllowed} onClick={() => startProtectedPlan('buy')} className="h-5 rounded border border-[#1c5c47] px-1.5 text-[6px] font-bold text-[#42D7A1] disabled:opacity-30">BUY plan</button>
              </div>
            )}
          </div>
