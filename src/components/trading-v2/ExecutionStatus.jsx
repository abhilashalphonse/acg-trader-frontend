...
export default function ExecutionStatus({ event, onDismiss = () => {} }) {
  ...
  const Icon = status === 'submitting' ? Loader2 : danger ? CircleAlert : status === 'accepted' ? Radio : Check;
  ...
  return (
    <div className={`fixed left-1/2 top-[72px] z-[140] w-[calc(100%-24px)] max-w-[430px] -translate-x-1/2 overflow-hidden rounded-xl border shadow-[0_16px_50px_rgba(0,0,0,.48)] backdrop-blur-xl ${danger ? 'border-[#64313a] bg-[#28141a]/96' : 'border-[#24445a] bg-[#0a1a26]/96'}`}>
      <div className="flex items-center gap-2.5 px-3 py-2.5">
        <div className={`grid size-8 shrink-0 place-items-center rounded-lg ${danger ? 'bg-[#3a1720] text-[#ff7a85]' : positive ? 'bg-[#0d3026] text-[#4de0ab]' : 'bg-[#10283a] text-[#64caff]'}`}>
          <Icon size={14} className={status === 'submitting' ? 'animate-spin' : ''}/>
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2"><b className={`text-[10px] ${danger ? 'text-[#ff8a93]' : 'text-[#edf4f8]'}`}>{labels[status] || status}</b><span className={`rounded-md px-1.5 py-0.5 text-[7px] font-black ${String(event.side).toUpperCase() === 'BUY' ? 'bg-[#0b3529] text-[#42d9a5]' : 'bg-[#391820] text-[#ff737e]'}`}>{String(event.side || '').toUpperCase()}</span><span className="truncate text-[8px] font-semibold text-[#7790a4]">{event.symbol}</span></div>
          ...
          {event.message && <p className={`mt-1 text-[8px] ${danger ? 'text-[#d99aa1]' : 'text-[#86a0b4]'}`}>{event.message}</p>}
        </div>
