...
function DrawingShape({ drawing, selected, onSelect, onStartHandle }) {
  ...
  if (drawing.type === 'hline') {
    return <line x1="0" y1={`${a.y * 100}%`} x2="100%" y2={`${a.y * 100}%`} {...lineHitProps(selected, '#f0c35c')} {...common} />;
  }

  if (drawing.type === 'vline') {
    return <line x1={`${a.x * 100}%`} y1="0" x2={`${a.x * 100}%`} y2="100%" {...lineHitProps(selected, '#f0c35c')} {...common} />;
  }
  ...
  if (drawing.type === 'rectangle') {
    ...
    return (
      <g>
        <rect x={`${x * 100}%`} y={`${y * 100}%`} width={`${width * 100}%`} height={`${height * 100}%`} fill="rgba(83,199,255,0.07)" stroke={selected ? '#ffffff' : '#53c7ff'} strokeWidth={selected ? 2 : 1.2} vectorEffect="non-scaling-stroke" {...common} />
        {selected && <><Handle x={a.x} y={a.y} onPointerDown={event => onStartHandle(event, drawing.id, 'a')} /><Handle x={b.x} y={b.y} onPointerDown={event => onStartHandle(event, drawing.id, 'b')} /></>}
      </g>
    );
  }
  ...
  if (drawing.type === 'text') {
    return (
      <g {...common}>
        <text x={`${a.x * 100}%`} y={`${a.y * 100}%`} fill={selected ? '#ffffff' : '#d8e4ee'} fontSize="10" fontWeight="600" style={{ paintOrder: 'stroke', stroke: '#071019', strokeWidth: 3 }}>{drawing.text || 'Text'}</text>
        {selected && <Handle x={a.x} y={a.y} onPointerDown={event => onStartHandle(event, drawing.id, 'a')} />}
      </g>
    );
  }
  ...
  return (
    <g>
      <line x1={`${a.x * 100}%`} y1={`${a.y * 100}%`} x2={`${b.x * 100}%`} y2={`${b.y * 100}%`} {...lineHitProps(selected)} {...common} />
      {selected && <><Handle x={a.x} y={a.y} onPointerDown={event => onStartHandle(event, drawing.id, 'a')} /><Handle x={b.x} y={b.y} onPointerDown={event => onStartHandle(event, drawing.id, 'b')} /></>}
    </g>
  );
}
...
export default function DrawingLayer({ symbol, timeframe, tool = 'cursor', onToolChange = () => {}, disabled = false }) {
  ...
  const pointerMove = event => {
    ...
    if (draft) {
      setDraft(current => current ? { ...current, b: point } : current);
      return;
    }
    ...
    setDrawings(current => current.map(item => {
      ...
      return {
        ...item,
        a: { x: clamp01(drag.original.a.x + dx), y: clamp01(drag.original.a.y + dy) },
        b: { x: clamp01(drag.original.b.x + dx), y: clamp01(drag.original.b.y + dy) },
      };
    }));
  };
  ...
  const finishPointer = event => {
    if (draft) {
      ...
      const distance = Math.hypot(created.b.x - created.a.x, created.b.y - created.a.y);
      if (distance > 0.015) {
        setDrawings(current => [...current, created]);
        setSelectedId(created.id);
      }
  ...
  return (
    <div className={`absolute inset-0 z-[16] ${drawingTool || selectedId ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      <svg
        ref={svgRef}
      ...
      {selected && !disabled && (
        <div className="absolute right-2 top-11 z-20 flex items-center gap-1 rounded-lg border border-[#243746] bg-[#08131d]/94 p-1 shadow-xl">
