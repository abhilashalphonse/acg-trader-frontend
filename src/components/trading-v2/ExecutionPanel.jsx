      ...
      return (
        <section className="relative shrink-0 border-t border-[#1a2b3a] bg-[#071019]/98 px-2 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 shadow-[0_-12px_34px_rgba(0,0,0,0.26)] backdrop-blur-xl">
          <div className="flex min-h-[52px] items-center gap-2">
            <button type="button" onClick={onCancelPlan} className="grid size-10 shrink-0 place-items-center rounded-xl border border-[#253746] bg-[#0d1822] text-[#91a3b5]" aria-label={isOpen ? 'Close demo position' : 'Cancel plan'}><X size={15}/></button>
            ...
            {isOpen ? <button type="button" onClick={() => onModifyPlan(isModifying ? 'open' : 'modifying')} className="h-10 shrink-0 rounded-xl border border-[#294054] bg-[#0d1a25] px-3 text-[10px] font-bold text-[#dbe5ed]"><SlidersHorizontal size={13} className="mr-1 inline"/>{isModifying ? 'Done' : 'Modify'}</button> : <button type="button" onClick={onExecutePlan} className={`h-10 shrink-0 rounded-xl px-3 text-[9px] font-black ${tradePlan.side === 'buy' ? 'border border-[#16865f] bg-[#0c5b45] text-[#6df0bd]' : 'border border-[#8a2b39] bg-[#4a1b25] text-[#ff818b]'}`}><Check size={13} className="mr-1 inline"/>{tradePlan.pending ? 'PLACE' : side}</button>}
          </div>
        </section>
      );
    }
  ...
  const compactControls = (
    <>
      ...
      <div className="mb-1.5 flex items-center gap-1.5">
        ...
        <span className="text-[8px] text-[#60758a]">{orderType === 'market' ? 'MT5 direct execution' : 'Pending order'}</span>
      </div>
      <div className={`grid ${focusMode ? 'grid-cols-[minmax(0,1fr)_94px_minmax(0,1fr)] gap-1.5' : 'grid-cols-[minmax(0,1fr)_100px_minmax(0,1fr)] gap-2'}`}>
        <button type="button" onClick={() => clickSide('sell')} className={`flex ${focusMode ? 'h-[58px] px-3' : 'h-[66px] px-4'} flex-col items-start justify-center rounded-[14px] border border-[#8a2b39] bg-gradient-to-br from-[#461b24] via-[#32131b] to-[#251017] text-left text-[#ff6975] active:scale-[0.99]`}><span className="text-[10px] font-extrabold tracking-[0.045em]">SELL</span><strong className={`${focusMode ? 'text-[21px]' : 'text-[27px]'} mt-1 font-black leading-none tracking-[-0.04em] text-[#f9f3f4]`}>{market.bid}</strong></button>
        ...
        <button type="button" onClick={() => clickSide('buy')} className={`flex ${focusMode ? 'h-[58px] px-3' : 'h-[66px] px-4'} flex-col items-end justify-center rounded-[14px] border border-[#16865f] bg-gradient-to-bl from-[#0b6048] via-[#0b4838] to-[#0b2e27] text-right text-[#44dda9] active:scale-[0.99]`}><span className="text-[10px] font-extrabold tracking-[0.045em]">BUY</span><strong className={`${focusMode ? 'text-[21px]' : 'text-[27px]'} mt-1 font-black leading-none tracking-[-0.04em] text-[#f3fbf8]`}>{market.ask}</strong></button>
      </div>
    </>
  );
