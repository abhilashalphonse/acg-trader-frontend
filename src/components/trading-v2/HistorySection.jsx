...
export default function HistorySection({ positionHistory = [], journal = [], markets = [], accountCurrency = 'USD', onOpenChart = () => {}, onNotice = () => {} }) {
  ...
  const filters = (
    <div className="relative mt-3 flex gap-2">
      <button type="button" onClick={() => { setPeriodOpen(v => !v); setSymbolOpen(false); }} className="flex h-9 items-center gap-1.5 rounded-md border border-white/[0.08] bg-[#080808] px-3 text-[8px] font-bold text-[#b5c2cd]">{period}<ChevronDown size={11}/></button>
      <button type="button" onClick={() => { setSymbolOpen(v => !v); setPeriodOpen(false); }} className="flex h-9 min-w-0 items-center gap-1.5 rounded-md border border-white/[0.08] bg-[#080808] px-3 text-[8px] font-bold text-[#b5c2cd]"><span className="max-w-[120px] truncate">{symbolFilter}</span><ChevronDown size={11}/></button>
  ...
  return (
    <section className="min-h-[calc(100dvh-98px)] px-3 pb-6 pt-3">
      ...
      <div className="grid grid-cols-3 border-y border-white/[0.08] bg-black">
        {[['deals', 'Deals'], ['orders', 'Orders'], ['session', 'Session']].map(([id, label]) => <button key={id} type="button" onClick={() => setTab(id)} className={`relative h-10 text-[9px] font-extrabold transition ${tab === id ? 'bg-[#101010] text-[#edf5fa] shadow-[0_5px_16px_rgba(0,0,0,.2)]' : 'text-[#71869a]'}`}>{label}{tab === id && <span className="absolute bottom-0.5 left-1/2 h-0.5 w-7 -translate-x-1/2 rounded-full bg-[#101010]"/>}</button>)}
      </div>

      {tab !== 'session' && filters}

      {tab === 'deals' && <>
        <div className="mt-4 border-y border-white/[0.08] bg-black py-3">
...
function Empty({ title, subtitle }) { return <div className="grid h-[120px] place-items-center border-y border-dashed border-white/[0.08] bg-black text-center"><div><b className="text-[10px] text-[#9eafbe]">{title}</b><p className="mt-1 text-[8px] text-[#5d7185]">{subtitle}</p></div></div>; }
