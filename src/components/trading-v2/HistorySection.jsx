...
export default function HistorySection({ positionHistory = [], journal = [], markets = [], accountCurrency = 'USD', onOpenChart = () => {}, onNotice = () => {} }) {
  ...
  const filters = (
    <div className="relative mt-3 flex gap-2">
      <button type="button" onClick={() => { setPeriodOpen(v => !v); setSymbolOpen(false); }} className="flex h-9 items-center gap-1.5 rounded-xl border border-white/[0.08] bg-[#080808] px-3 text-[8px] font-bold text-[#b5c2cd]">{period}<ChevronDown size={11}/></button>
      <button type="button" onClick={() => { setSymbolOpen(v => !v); setPeriodOpen(false); }} className="flex h-9 min-w-0 items-center gap-1.5 rounded-xl border border-white/[0.08] bg-[#080808] px-3 text-[8px] font-bold text-[#b5c2cd]"><span className="max-w-[120px] truncate">{symbolFilter}</span><ChevronDown size={11}/></button>
  ...
  return (
    <section className="min-h-[calc(100dvh-98px)] px-3 pb-6 pt-3">
      <header className="flex items-start justify-between gap-3 pb-3">
        ...
        <button type="button" onClick={shareSummary} className="mt-1 grid size-10 place-items-center rounded-xl border border-white/[0.08] bg-[#101010] text-[#63cbff]" aria-label="Share trading performance"><Share2 size={16}/></button>
      </header>

      <div className="grid grid-cols-3 rounded-[15px] border border-white/[0.08] bg-[#080808] p-1">
        {[['deals', 'Deals'], ['orders', 'Orders'], ['session', 'Session']].map(([id, label]) => <button key={id} type="button" onClick={() => setTab(id)} className={`relative h-10 rounded-xl text-[9px] font-extrabold transition ${tab === id ? 'bg-[#101010] text-[#edf5fa] shadow-[0_5px_16px_rgba(0,0,0,.2)]' : 'text-[#71869a]'}`}>{label}{tab === id && <span className="absolute bottom-0.5 left-1/2 h-0.5 w-7 -translate-x-1/2 rounded-full bg-[#101010]"/>}</button>)}
      </div>

      {tab !== 'session' && filters}

      {tab === 'deals' && <>
        <div className="mt-4 rounded-[22px] border border-white/[0.08] bg-[#080808] p-4 shadow-[0_18px_50px_rgba(0,0,0,.22)]">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 border-t border-white/[0.08] pt-3"><SummaryStat label="Deals" value={filteredDeals.length}/><SummaryStat label="W / L" value={`${stats.wins} / ${stats.losses}`}/><SummaryStat label="Volume" value={`${stats.lots.toFixed(2)} lots`}/></div>
        ...
        <div className="mt-4 space-y-2">
          ...
          {filteredDeals.map(deal => {
            ...
            return <article key={deal.id} className="overflow-hidden rounded-[18px] border border-white/[0.08] bg-[#080808]"><button type="button" onClick={() => setExpandedId(expanded ? null : deal.id)} className="w-full px-3.5 py-3 text-left"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><InstrumentAvatar instrument={instrumentForSymbol(markets, deal.symbol)} size={24}/><strong className="text-[12px] font-black text-[#f0f5f8]">{symbolLabel(deal.symbol)}</strong><span className={`rounded-md px-1.5 py-1 text-[7px] font-black ${sideTone(deal.side)}`}>{String(deal.side).toUpperCase()} · {Number(deal.volume).toFixed(2)}</span></div><p className="mt-2 text-[8px] text-[#5f7488]">{deal.closedAt || 'Executed'} · {deal.closeType || 'CLOSE'}</p></div><b className={`text-[15px] font-black ${positive ? 'text-[#43d9a6]' : 'text-[#ff6f7a]'}`}>{money(deal.pnl, accountCurrency, true)}</b></div></button>{expanded && <div className="border-t border-white/[0.08] bg-[#080808] p-3.5"><div className="grid grid-cols-2 gap-3"><Detail label="Execution" value={formatInstrumentPrice(deal.closePrice ?? deal.entry, instrumentForSymbol(markets, deal.symbol))}/><Detail label="Commission" value={money(deal.commission || 0, accountCurrency)}/><Detail label="Swap" value={money(deal.swap || 0, accountCurrency)}/><Detail label="Type" value={deal.closeType || 'CLOSE'}/></div><button type="button" onClick={() => onOpenChart(deal.symbol)} className="mt-3 h-10 w-full rounded-xl border border-white/[0.08] bg-[#101010] text-[9px] font-bold text-[#62cbff]">View on Chart</button></div>}</article>;
          })}
        </div>
      </>}
      ...
      {tab === 'orders' && <div className="mt-4 space-y-2">
        ...
        {filteredOrders.map(order => <article key={order.id} className="rounded-[18px] border border-white/[0.08] bg-[#080808] px-3.5 py-3"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><InstrumentAvatar instrument={instrumentForSymbol(markets, order.symbol)} size={24}/><strong className="text-[12px] font-black text-[#f0f5f8]">{symbolLabel(order.symbol)}</strong><span className={`rounded-md px-1.5 py-1 text-[7px] font-black ${sideTone(order.side)}`}>{String(order.side).toUpperCase()}</span></div><p className="mt-2 text-[8px] text-[#718397]">{order.type} · {Number(order.requestedVolume || 0).toFixed(2)} lots</p><p className="mt-1 text-[8px] text-[#53697d]">{eventDate(order) ? new Date(eventDate(order)).toLocaleString() : '—'}</p></div><span className="rounded-lg border border-white/[0.08] bg-[#101010] px-2 py-1 text-[8px] font-black text-[#9db0c1]">{order.status}</span></div></article>)}
      </div>}

      {tab === 'session' && <div className="mt-4"><div className="px-1"><h2 className="text-[11px] font-black text-[#e8eff5]">Terminal session activity</h2><p className="mt-1 text-[8px] text-[#60758a]">Actions from this browser session. Executed records remain in Deals and Orders.</p></div><div className="mt-2 space-y-2">{!journal.length && <Empty title="No session activity yet" subtitle="Commands submitted in this browser session will appear here."/>}{journal.map(event => <article key={event.id} className="rounded-[17px] border border-white/[0.08] bg-[#080808] px-3.5 py-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><strong className="truncate text-[11px] font-black text-[#eef4f8]">{symbolLabel(event.symbol) || 'Account'}</strong><p className="mt-2 text-[9px] leading-4 text-[#8da0b1]">{event.message}</p></div><span className="shrink-0 text-[8px] font-semibold text-[#596f83]">{event.time || '—'}</span></div></article>)}</div></div>}
    </section>
  );
}

function Menu({ children, className = '' }) { return <div className={`absolute z-40 min-w-[132px] rounded-xl border border-white/[0.08] bg-[#080808] p-1.5 shadow-[0_18px_50px_rgba(0,0,0,.55)] ${className}`}>{children}</div>; }
function MenuItem({ children, active, onClick }) { return <button type="button" onClick={onClick} className={`flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-left text-[9px] font-semibold ${active ? 'bg-[#101010] text-[#61caff]' : 'text-[#b4c1cc]'}`}><span>{children}</span>{active && <Check size={11}/>}</button>; }
...
function Empty({ title, subtitle }) { return <div className="grid h-[120px] place-items-center rounded-[18px] border border-dashed border-white/[0.08] bg-[#080808] text-center"><div><b className="text-[10px] text-[#9eafbe]">{title}</b><p className="mt-1 text-[8px] text-[#5d7185]">{subtitle}</p></div></div>; }
