...
export default function TradingChart({
  ...
  const clearIndicatorSeries = useCallback(chart => {
    ...
    indicatorSeriesRef.current.forEach(series => {
      try { chart.removeSeries(series); } catch (_) { /* already disposed */ }
    });
    indicatorSeriesRef.current = [];
    indicatorBindingsRef.current = [];
    for (let index = indicatorPanesRef.current; index >= 1; index -= 1) {
      try { if (chart.panes().length > index) chart.removePane(index); } catch (_) { /* pane already gone */ }
    }
  ...
  const updateIndicatorData = useCallback(bars => {
    ...
    indicatorBindingsRef.current.forEach(binding => {
      ...
      if (binding.histogram && result.histogram) {
        binding.histogram.setData(result.histogram.map(point => ({ ...point, color: point.value >= 0 ? 'rgba(45,211,155,0.45)' : 'rgba(255,95,105,0.45)' })));
      }
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const chart = createChart(hostRef.current, {
      ...
      layout: {
        ...
        fontSize: 10,
        panes: { separatorColor: '#1b2b39', separatorHoverColor: 'rgba(83,199,255,0.18)', enableResize: true },
      },
      grid: {
      ...
      crosshair: {
        ...
        vertLine: { visible: true, color: chartTokens.crosshair, width: 1, style: LineStyle.Dashed, labelVisible: true, labelBackgroundColor: chartTokens.crosshairLabel },
        horzLine: { visible: true, color: chartTokens.crosshair, width: 1, style: LineStyle.Dashed, labelVisible: true, labelBackgroundColor: chartTokens.crosshairLabel },
      },
    ...
    const series = chartMode === 'line'
      ? chart.addSeries(LineSeries, { color: chartTokens.blue, lineWidth: 2, priceFormat, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: true })
      : chart.addSeries(CandlestickSeries, { upColor: chartTokens.buy, downColor: chartTokens.sell, wickUpColor: chartTokens.buy, wickDownColor: chartTokens.sell, borderVisible: false, priceFormat, priceLineVisible: false, lastValueVisible: false });
    ...
    const crosshairHandler = param => {
      if (!param?.time) { setDisplayBar(lastBarRef.current); return; }
    ...
    (async () => {
      try {
        ...
        volume.setData(bars.map(bar => ({ time: bar.time, value: volumeForBar(bar), color: bar.close >= bar.open ? 'rgba(45,211,155,0.34)' : 'rgba(255,95,105,0.32)' })));
        lastBarRef.current = bars[bars.length - 1];
        setDisplayBar(bars[bars.length - 1]);
        renderIndicators(chart, bars);
        chart.timeScale().setVisibleLogicalRange({ from: Math.max(0, bars.length - 48), to: bars.length + 9 });
      } catch (e) {
  ...
  useEffect(() => {
    chartRef.current?.applyOptions({ grid: { vertLines: { visible: showGrid, color: chartTokens.gridline, style: LineStyle.Dotted }, horzLines: { visible: showGrid, color: chartTokens.gridline, style: LineStyle.Dotted } } });
  }, [showGrid]);

  useEffect(() => { volumeRef.current?.applyOptions({ visible: showVolume }); }, [showVolume, symbol, timeframe, chartMode]);

  useEffect(() => {
    chartRef.current?.timeScale().applyOptions({ rightOffset: chartShift ? 10 : 2, barSpacing });
  ...
  useEffect(() => {
    ...
    if (Number.isFinite(liveBid)) {
      if (!bidLineRef.current) bidLineRef.current = series.createPriceLine({ price: liveBid, color: 'rgba(45,211,155,0.82)', lineWidth: 1, lineStyle: LineStyle.Dotted, axisLabelVisible: false, title: '' });
      else bidLineRef.current.applyOptions({ price: liveBid });
    }

    if (Number.isFinite(liveAsk)) {
      if (!askLineRef.current) askLineRef.current = series.createPriceLine({ price: liveAsk, color: 'rgba(255,95,105,0.62)', lineWidth: 1, lineStyle: LineStyle.Dashed, axisLabelVisible: true, title: 'ASK' });
      else askLineRef.current.applyOptions({ price: liveAsk });
    }
  ...
  const resetView = () => {
    ...
    if (!chart || !bars.length) return;
    setBarSpacing(7);
    chart.timeScale().setVisibleLogicalRange({ from: Math.max(0, bars.length - 48), to: bars.length + (chartShift ? 9 : 2) });
  };
  ...
  return (
    <div className="relative size-full min-h-0 min-w-0 overflow-hidden bg-[#080f17]">
      ...
      <div className="absolute right-[54px] top-2 z-30 flex items-center gap-0.5 rounded-lg border border-[#203241] bg-[#08131d]/88 p-1 shadow-[0_5px_20px_rgba(0,0,0,0.22)] backdrop-blur-md">
        ...
        <button type="button" title="Volumes" aria-label="Toggle volumes" onClick={() => setShowVolume(v => !v)} className={toolButton(showVolume)}><BarChart3 size={14} /></button>
        <button type="button" title="Auto scroll" aria-label="Toggle auto scroll" onClick={() => setAutoScroll(v => !v)} className={toolButton(autoScroll)}>{autoScroll ? <Play size={13} /> : <Pause size={13} />}</button>
        <button type="button" title="Chart shift" aria-label="Toggle chart shift" onClick={() => setChartShift(v => !v)} className={toolButton(chartShift)}><span className="text-[9px] font-black leading-none">⇥</span></button>
        <button type="button" title="Zoom out" aria-label="Zoom out" onClick={() => setBarSpacing(v => Math.max(3, +(v - 1.25).toFixed(2)))} className={toolButton(false)}><ZoomOut size={14} /></button>
        ...
        <button type="button" title="Reset chart view" aria-label="Reset chart view" onClick={resetView} className={toolButton(false)}><RotateCcw size={13} /></button>
      </div>
